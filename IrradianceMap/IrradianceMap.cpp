...
void IrradianceMap::ParseCommandLineArgs(wchar_t* argv[], int argc)
{
	...
	for (auto i = 1; i < argc; ++i)
	{
		...
		else if (isArgMatched(i, L"env"))
		{
			...
			while (hasNextArgValue(i)) m_envFileNames.emplace_back(argv[++i]);
		}
		else if (isArgMatched(i, L"gt"))
		{
			m_envFileNames.clear();
			if (hasNextArgValue(i)) m_envFileNames.emplace_back(argv[++i]);
...
void IrradianceMap::SaveImage(char const* fileName, Buffer* pImageBuffer, uint32_t w, uint32_t h, uint32_t rowPitch, uint8_t comp)
{
